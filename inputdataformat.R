...
vcffile<-as.character(args[1])	#AM: if running line by line, do vcffile<-as.character("file.vcf")
#vcffile<-as.character("EGM19_target_27jul22_miss0.4_mac3_Q30_DP3.recode.vcf")
...
if(mult.files){
	...
	for(i in 1:nrow(a.pl)){ #AM: rows=loci, cols=inds
		for(j in 1:ncol(a.pl)){
			if(is.na(a.pl[i,j])){	#AM: problem is here with dimensionality (thinks that a.gl is 2D but it's actually 3D). changing gl to pl on 124 
	...
	for(locus in 1:nloci){
		...
		write.table(unlist(t(a.gl[ ,locus,])), file=paste0(fname, ".mpgl"), append=T, #AM: added , after locus,
					col.names=F, row.names=F, eol=" ")
	}
}
...
if(mult.files){
	...
	a.mean.gl<-apply(a.gl, c(2,3), function(x){temp<-sum(10^(-0.1*unlist(x)));sum((10^(-0.1*unlist(x)))*(0:max(ploidy)))/temp}) # changed c(1,2) to c(2,3)
}
...
pcSummary<-summary(pcout) #AM: plot proportion of variance (middle row) for PC1 and PC2 of pcout object
